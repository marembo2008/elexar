...
public class ElectiveRegionFacade extends AbstractFacade<ElectiveRegion> {
  ...
  private long normalizeRegisteredVoters(ElectiveRegion electiveRegion) {
    if (electiveRegion.getElectiveRegionType() == ElectiveRegionType.COUNTY_WARD) {
      ...
      String sql = "SELECT SUM(P.REGISTEREDVOTERS) FROM POLLINGSTATION P, COUNTYWARD W WHERE P.PARENTELECTIVEREGION_REGIONID = W.REGIONID AND W.REGIONID = " + electiveRegion.getRegionId();
      ...
      if (pollingSumTotal != null) {
        ...
        edit(electiveRegion);
        System.err.println("normalizeRegisteredVoters: " + electiveRegion + ": " + electiveRegion.getElectiveRegionType() + ": " + pollingSumTotal);
