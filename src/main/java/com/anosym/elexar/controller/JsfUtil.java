...
public class JsfUtil {
  ...
  public static SelectItem[] getSelectItems(List<?> entities, boolean selectOne) {
    ...
    int i = 0;
    if (selectOne) {
      items[0] = new SelectItem("", "Select Category");
      i++;
    }
  ...
  public static SelectItem[] getSelectItems(List<?> entities, boolean selectOne, String selectOneUnselectMessage) {
    int size = selectOne ? entities.size() + 1 : entities.size();
    SelectItem[] items = new SelectItem[size];
    int i = 0;
    if (selectOne) {
      items[0] = new SelectItem("", selectOneUnselectMessage);
      i++;
    }
    for (Object x : entities) {
      if (x != null) {
